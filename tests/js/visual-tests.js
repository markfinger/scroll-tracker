(function() {
  ...
  var initscrollOffsets = function() {
    scroll.settings.topPadding = viewportTopElement.outerHeight();
    scroll.settings.bottomPadding = viewportBottomElement.outerHeight();
  };

  var insertElements = function() {
    var conditions = _.keys(scroll.conditions);
  ...
  var bindElements = function() {
    _.each(elements, function(elementsToBind, binding) {
      _.each(elementsToBind, function($element) {
        ...
        scroll.on(element, binding, function() {
          if (!animating) {
  ...
  var setBindings = function() {
    $(window).on('resize', _.debounce(initscrollOffsets, 10));
